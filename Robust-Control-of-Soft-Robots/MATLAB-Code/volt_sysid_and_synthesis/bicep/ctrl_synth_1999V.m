...
%% performance
% % ss
rectangle('Position',[1 40 0.2 0], 'LineWidth',1);

% % tracking
rectangle('Position',[1 0 9 24], 'LineWidth',1);

% % tracking 2
rectangle('Position',[1 0 19 17],'LineWidth',1);

% tracking 3
rectangle('Position',[1 0 39 10],'LineWidth',1);

% % bandwidth
rectangle('Position',[100 -5 0 10], 'LineWidth',1);

% % disturbance rejection
rectangle('Position',[180 -5 700 5], 'LineWidth',1);

% % disturbance rejection
rectangle('Position',[300 -10 700 10], 'LineWidth',1);
%%
...
f7 = figure(7)
s = tf('s');
sys_tricep = load('tricep_tfs_1999V.mat')
hold on
for m = 1:3
    % multiplicative uncertainty
    sysc{m} = d2c(sys_tricep.sys{m});
    g_err{m} = (sysc{m} - g )/ g ;
    h = sigmaplot(g_err{m});
    hold on
