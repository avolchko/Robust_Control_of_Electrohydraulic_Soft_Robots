...
def calculate(bottom):
    ...
    disp = act.initial_length - length

    # publish disp in mm
    pub.publish(disp*1000)

def start():
    # initialize node
    rospy.init_node('disp_node_triceps')
    ...
    global pub	
    pub = rospy.Publisher('disp_triceps', Float32, queue_size = 1)
    ...
    rospy.Subscriber("mocap_node/tricep_bottom/pose", PoseStamped, calculate)
